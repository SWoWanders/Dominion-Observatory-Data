...
(function() {
    ...
    const factsheet = {
                ...
                list.innerHTML = all.map(fact => `
                    <div class="factsheet-item" draggable="true" data-fact-id="${escapeAttr(fact.id)}">
                        ...
                        <div class="factsheet-item-body">
                            <div class="factsheet-item-context">${escapeHtml(formatCategoryLabel(fact.category))} &middot; ${escapeHtml(fact.itemName || '')}</div>
        ...
        openCardPopover(card, item, category) {
            ...
            popover.innerHTML = `
                <div class="factsheet-popover-header">
                    <strong>Add facts from "${escapeHtml(item.name || item.title || item.question || 'this item')}"</strong>
            ...
            // Show the popover as a centered modal-style overlay. (Earlier versions
            // tried to anchor it to the card, but that broke on mobile and on long
            // pages when the card was near the bottom of the viewport — the popover
            // would extend off-screen and the checkboxes were unreachable.)
            popover.style.position = 'fixed';
            popover.style.zIndex = '10000';
            popover.style.left = '50%';
            popover.style.top = '50%';
            popover.style.transform = 'translate(-50%, -50%)';
            popover.style.width = Math.min(440, window.innerWidth - 32) + 'px';
            popover.style.maxHeight = '80vh';

            // Backdrop that dims the page and catches outside clicks
            const backdrop = document.createElement('div');
            backdrop.id = 'factsheet-popover-backdrop';
            ...
            // Close-on-backdrop-click and close-on-Escape
            const closePopover = () => {
                popover.remove();
                backdrop.remove();
            ...
            // Wire up handlers — listen on both the checkbox and the wrapping label.
            // On some mobile browsers, change events inside <label> elements can be
            // delivered inconsistently; the click handler on the label is a reliable
            // fallback. We use a Set to dedupe so a single tap doesn't toggle twice.
            popover._facts = facts;
            const recentlyToggled = new Set();
            const handleToggle = (factId, sourceCheckbox) => {
                if (!factId) return;
                if (recentlyToggled.has(factId)) return;
                recentlyToggled.add(factId);
                setTimeout(() => recentlyToggled.delete(factId), 100);
                const factData = facts.find(f => f.id === factId);
                if (factData) {
                    factsheet.toggle(factData);
                    // Sync the checkbox visual state to the current store
                    if (sourceCheckbox) {
                        sourceCheckbox.checked = factsheet.has(factId);
                    }
                }
            };
            popover.querySelectorAll('input[type="checkbox"]').forEach(cb => {
                cb.addEventListener('change', (e) => {
                    handleToggle(e.target.getAttribute('data-fact-id'), e.target);
                });
            });
            // Also catch label clicks for extra robustness on mobile
            popover.querySelectorAll('.factsheet-fact-row').forEach(row => {
                row.addEventListener('click', (e) => {
                    // If the click target IS the checkbox, the change handler covers it
                    if (e.target.tagName === 'INPUT') return;
                    const cb = row.querySelector('input[type="checkbox"]');
                    if (!cb) return;
                    // Toggle the checkbox visually and trigger our handler
                    // Don't preventDefault — the label's native behavior already toggles
                    // the checkbox; we just need to make sure our handler runs.
                    setTimeout(() => {
                        handleToggle(cb.getAttribute('data-fact-id'), cb);
                    }, 0);
                });
            });

            popover.querySelector('.factsheet-popover-close').addEventListener('click', closePopover);
        },
        ...
        print() {
            ...
            const byCategory = {};
            all.forEach(fact => {
                let cat = capitalize(fact.category || 'Other');
                // Special-case category labels so they read naturally as headings
                if (cat === 'Faqs') cat = 'FAQ';
                if (cat === 'Discoveries') cat = 'Discoveries & Achievements';
    ...
    function collectFactsFromItem(item, category) {
        const facts = [];
        const itemId = item.id || item.title || item.name || item.question || ('item-' + Math.random());
        const itemName = item.name || item.title || item.shortName || item.question || 'Unknown';
        ...
        // Always offer the name (or question, for FAQ items)
        if (item.name || item.title) {
            add('name', 'Name', item.name || item.title);
        } else if (item.question) {
            add('question', 'Question', item.question);
        }

        // Walk the rest of the top-level fields
        Object.keys(item).forEach(key => {
            if (['id', 'name', 'title', 'question', 'citations', 'researchNote', 'shortName'].indexOf(key) >= 0) return;
    ...
    // Maps raw category keys to nicely-formatted display labels used in the
    // fact sheet panel and in the printed output. Special cases handle the
    // awkward English plurals and the renamed Discoveries & Achievements
    // category.
    function formatCategoryLabel(category) {
        if (!category) return '';
        if (category === 'faqs') return 'FAQ';
        if (category === 'discoveries') return 'Discoveries & Achievements';
    ...
    function wirePanelEvents() {
        ...
        // Defensive: ensure the panel always starts CLOSED on page load.
        // Some browsers (notably Chrome on Android) can restore DOM state from
        // a previous session when re-opening tabs, and we don't want a leftover
        // 'open' class to greet a returning user with the panel mid-screen.
        // We do this three times for paranoid robustness:
        //   1. Immediately (now)
        //   2. After a microtask (catches any synchronous code that runs after us)
        //   3. After requestAnimationFrame (catches anything that runs before paint)
        const hadOpenClass = panel.classList.contains('open');
        panel.classList.remove('open');
        Promise.resolve().then(() => panel.classList.remove('open'));
        requestAnimationFrame(() => panel.classList.remove('open'));
        // Mark the panel JS-ready so the CSS-level "keep off-screen until ready"
        // guard releases. This guarantees the panel is closed before being made
        // visually controllable.
        panel.setAttribute('data-js-ready', '1');
        // Lightweight diagnostic so we can confirm this code path runs.
        // Visible only in the browser console (View → Developer → JavaScript Console,
        // or remote-inspect via chrome://inspect on desktop while phone is connected).
        if (window.console && console.info) {
            console.info('[factsheet] init: panel had .open class on load?', hadOpenClass);
        }
        ...
        // Close-on-Escape, like the splash overlay, for keyboard accessibility
